...
class CommandWrappers:
   ...
   # @publish_live("~get_gestures_ramp", msg.setGesture)
   # def getGestures_ramp():
    #    return msg.Gestures([
    #        msg.Gesture(
    #            name,
    #            vals['magnitude'],
    ...
    def getGestures():
        return msg.Gestures([
            msg.Gesture(name, vals['speed'], vals['magnitude'], vals['priority'], rospy.Duration(vals['duration'])) for name, vals in api.getGestures().items()])
    ...
    def setGesture(msg):
        if api.pauAnimationMode & api.PAU_ACTIVE > 0:
            return
        try:
            api.setGesture(msg.name, msg.repeat, msg.speed, msg.magnitude, msg.priority, msg.rampin, msg.rampout, msg.duration.to_sec(), msg.start)
        except TypeError:
