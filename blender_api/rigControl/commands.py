...
class EvaAPI(RigAPI):
    ...
    def getGestures(self):
        ...
        for gesture in eva.gesturesList:
            ...
            magnitude=round(gesture.speed, 3)
            emotionGestures[gesture.name] = {'duration': duration, \
                'magnitude': magnitude, 'speed': speed, 'priority':gesture.priority}
        return emotionGestures


    def setGesture(self, name, repeat=1, speed=0.5, magnitude=1, priority=1, rampin=1, rampout=1, duration=1, start=0):
        bpy.evaAnimationManager.newGesture("GST-"+name, repeat, speed, magnitude, priority, rampin, rampout, duration, start)
        return 0
