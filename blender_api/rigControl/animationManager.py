...
class AnimationManager():
    ...
    def newGesture(self, name, repeat = 1, speed=0.5, magnitude=1, priority=1, rampInRatio=1, rampOutRatio=1, duration=0, startTime=0):
        fail = False
        try:
        ...
        checkValue(duration, 0, 1)
        checkValue(magnitude, 0, 1)
        #checkValue(rampInRatio+rampOutRatio, 0, 2.0)
      # Create NLA track
        ...
        g = Gesture(name, newTrack, newStrip, duration=duration, speed=speed, magnitude=magnitude, priority=priority, repeat=repeat, influence_kfp=influence_kfp, rampInRatio=rampInRatio, rampOutRatio=rampOutRatio, startTime=startTime)
        self.gesturesList.append(g)
...
class Gesture():
    ...
    def __init__(self, name, track, strip, duration, speed, magnitude, priority, repeat, influence_kfp, rampInRatio, rampOutRatio, startTime):
        self.name = name
        ...
        self.trackRef = track
        self.stripRef = strip
        self.influence_kfp = influence_kfp 
        self.rampInRatio = rampInRatio
        self.rampOutRatio = rampOutRatio
        ...
        num.add_keyframe(target=1.0, time=startOut, transition=(0, Pipes.linear(rampInRatio )))
        num.add_keyframe(target=0.0, transition=(0, Pipes.linear(rampOutRatio )))
