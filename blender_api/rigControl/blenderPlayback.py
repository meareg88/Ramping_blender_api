    ...
    def modal(self, context, event):
        ...
        if event.type == 'TIMER':
            ...
            for gesture in gestures:
                ...
                gesture.stripRef.influence = gesture.magnitude.current
                gesture.magnitude.blend(time, dt)
                gesture.influence_kfp.co[1] += gesture.magnitude.current     

              ### The 'gesture.speed*timeScale' based blending in and out applied here can be might be improved with a better technique###

                #blending in     
                rampInRatio = max(gesture.rampInRatio * gesture.duration, 0.01) 
                rampOutRatio = max(gesture.rampOutRatio * gesture.duration, 0.01)    

                if gesture.stripRef.strip_time <= rampInRatio:
                    gesture.stripRef.strip_time += gesture.speed * timeScale * (gesture.rampInRatio)
                    #print(gesture.name)
                    #print('gesture.stripRef.strip_time blending in')
                    #print(gesture.stripRef.strip_time )

                # after blending in and before blending out
                if rampInRatio < gesture.stripRef.strip_time <= gesture.duration-(gesture.duration - rampOutRatio):
                    gesture.stripRef.strip_time += gesture.speed * timeScale
                    #print('gesture.stripRef.strip_time b/n blending')
                    #print(gesture.stripRef.strip_time )

                # blending out
                if gesture.duration-rampOutRatio < gesture.stripRef.strip_time <= gesture.duration:
                    gesture.stripRef.strip_time += gesture.speed * timeScale * (gesture.rampOutRatio)
                    #print('gesture.stripRef.strip_time blending out')
                    #print(gesture.stripRef.strip_time )
